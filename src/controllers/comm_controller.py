...
class CommController:
    ...
    def __find(self) -> None:
        self.__port_refresh = self.__parent.findChild(
            QPushButton, "port_refresh_button"
        )
        self.__client_options_box = self.__parent.findChild(
            QComboBox, "client_options_box"
        )

    def __connect(self) -> None:
        self.__port_refresh.clicked.connect(self.__port_refresh_func)
        self.__client_options_box.currentTextChanged.connect(
            self.__client_options_box_func
        )
    ...
    def __port_refresh_func(self) -> None:
        ...
        def after_thread_func() -> None:
            ...
            self.__parent.findChild(QComboBox, "port_options_box").addItems(
                find_ports()
            )
