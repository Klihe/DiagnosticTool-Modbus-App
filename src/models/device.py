...
class Device:
    ...
    def set_working_state(func: Callable[['Device', Any], Any]) -> Callable[['Device', Any], None]:
    ...
    @set_working_state
    def connect(self, method: str, port: str, baudrate: str, bytesize: str, parity: str, stopbits: str) -> None:
    ...
    def __read_item(self, read_func: callable, number_of_items: int, bits: Optional[bool]=False, registers: Optional[bool]=False) -> None:
        address = 0
        response = []
        ...
        for _ in range(full_chunks):
            curr_response = read_func(address=address, count=self.__config["device"]["max_count"], slave=1)
            if bits:
